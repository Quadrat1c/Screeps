// This task will react on yellow/red flags, sending a melee attacker creep to the flags position.
let mod = {};
module.exports = mod;
mod.name = 'melee';
...
mod.handleFlagFound = flag => {
    // if it is a yellow/red flag
    if (flag.compareTo(FLAG_COLOR.melee) && Task.nextCreepCheck(flag, mod.name)) {
        Util.set(flag.memory, 'task', mod.name);
        // check if a new creep has to be spawned
        Task.melee.checkForRequiredCreeps(flag);
    }
};
mod.creep = {
    melee: {
...
mod.checkForRequiredCreeps = (flag) => {
    // get task memory
    let memory = Task.melee.memory(flag);
    ...
    if( count < 1 ) {
        Task.spawn(
            Task.melee.creep.melee, // creepDefinition
            { // destiny
                task: 'melee', // taskName
            ...
            creepSetup => { // callback onQueued
                let memory = Task.melee.memory(Game.flags[creepSetup.destiny.targetName]);
                memory.queued.push({
...
mod.handleSpawningStarted = params => { // params: {spawn: spawn.name, name: creep.name, destiny: creep.destiny}
    ...
    if ( !params.destiny || !params.destiny.task || params.destiny.task != 'melee' )
        return;
...
mod.handleSpawningCompleted = creep => {
    ...
    if (!creep.data || !creep.data.destiny || !creep.data.destiny.task || creep.data.destiny.task != 'melee')
        return;
...
mod.handleCreepDied = name => {
    ...
    // ensure it is a creep which has been requested by this task (else return)
    if (!mem || !mem.destiny || !mem.destiny.task || mem.destiny.task != 'melee')
        return;
    // get flag which caused request of that creep
    let flag = Game.flags[mem.destiny.flagName];
    if (flag) {
        const memory = Task.melee.memory(flag);
...
mod.memory = (flag) => {
    if( !flag.memory.tasks ) 
        flag.memory.tasks = {};
    if( !flag.memory.tasks.melee ) {
        flag.memory.tasks.melee = {
            queued: [], 
            spawning: [],
            running: []
        };
    }
    return flag.memory.tasks.melee;
};

...
{
    let threat = this.find(FIND_HOSTILE_CREEPS, {filter: (c) => c.getActiveBodyparts(ATTACK)}.length, {filter: (c) => c.getActiveBodyparts(RANGED_ATTACK)}).length;
    if (threat > 0)
    {
        this.memory.defConMode = 'active'
        console.log(txt(COLOR.warn, this.name + ' Threat: ' + threat + ' DefCon Mode Activated.'));
    }
    else
    {

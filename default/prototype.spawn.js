...
{
    //console.log(room.energyCapacityAvailable / 200);
    if(room.memory.defConMode == 'active') { console.log(txt(COLOR.warn, 'DefCon is Active.')); }
    ...
    if (this.room.memory.creeps.harvesters < 1 && this.room.memory.creeps.miners < 2 && room.energyAvailable < 650)
    {
        ...
        else if(ret != ERR_NOT_ENOUGH_ENERGY)  { console.log(txt(COLOR.error, 'Error spawning miner: ' + ret)); }
        return;
    }
    ...
    if (this.room.memory.creeps.harvesters > 0 && this.room.memory.creeps.haulers < 1 && room.energyAvailable <= 300)
    {
        ...
        else if(ret != ERR_NOT_ENOUGH_ENERGY) { console.log(txt(COLOR.error, 'Error spawning hauler: ' + ret)); }
        return;
    }
    ...
    for(let id of this.room.memory.sources)
    {
        ...
        if(source.memory.harvester != 'active')
        {
            ...
            else if(ret != ERR_NOT_ENOUGH_ENERGY) { console.log(txt(COLOR.error, 'Error spawning harvester: ' + ret)); }
            return;
        }
    }
    ...
    if(room.memory.storage && room.memory.energyConMode >= 1 && room.memory.creeps.suppliers < 1)
    {
        ...
        if(storageStructure)
        {
            ...
            else if(ret != ERR_NOT_ENOUGH_ENERGY) { console.log(txt(COLOR.error, 'Error spawning supplier: ' + ret)); }
            return;
        }
    }
    ...
    if (room.memory.creeps.repairers < repairerMax && room.getNumRepairTargets() >= 3)
    {
        ...
        else if(ret != ERR_NOT_ENOUGH_ENERGY) { console.log(txt(COLOR.error, 'Error spawning repairer: ' + ret)); }
        return;
    }
    ...
    if(room.memory.defConMode == 'active' && room.memory.creeps.bouncers < 1)
    {
        ...
        else if(ret != ERR_NOT_ENOUGH_ENERGY) { console.log(txt(COLOR.error, 'Error spawning bouncer: ' + ret)); }
    }
    ...
    if(sites.length > 0 && room.memory.creeps.builders < maxBuilders)
    {
        ...
        else if(ret != ERR_NOT_ENOUGH_ENERGY) { console.log(txt(COLOR.error, 'Error spawning builder: ' + ret)); }
        return;
    }
    ...
    if(room.controller.level >= 5 && Game.getObjectById(room.memory.controllerLink) && Game.getObjectById(room.memory.spawnLink))
    {
          ...
          (room.controller.level < 8 && room.memory.energyConMode >= 5 && room.memory.creeps.upgraders < 2))
        {
            ...
            else if(ret != ERR_NOT_ENOUGH_ENERGY) { console.log(txt(COLOR.error, 'Error spawning upgrader: ' + ret)); }
            return;
        }
    }
    else 
    ...
    if(room.controller.level >= 6)
    {
        ...
        if(mineral.mineralAmount > 0 && room.memory.creeps.mineralMiners < maxMineralMiners)
        {
            ...
            else if(ret != ERR_NOT_ENOUGH_ENERGY) { console.log(txt(COLOR.error, 'Error spawning mineralMiner: ' + ret)); }
            return;
        }
    }
    ...
    if(manualSpawn)
    {
        ...
        if (Memory.rooms[name].creeps.rangers < 1)
        {
            ...
            else if(ret != ERR_NOT_ENOUGH_ENERGY) { console.log(txt(COLOR.error, 'Error spawning ranger: ' + ret)); }
            return;
        }
    }
    ...
    if(room.memory.storage && Game.getObjectById(room.memory.storage))
    {
        for(let name of this.room.memory.exitRooms)
        {
            ...
            // Spawn bouncer
            if(!Memory.rooms[name].neighborData.skLair && Memory.rooms[name].neighborData.hostile && Memory.rooms[name].creeps.bouncers < 1)
            {
                let ret = this.createBouncer(room.energyCapacityAvailable * 0.4, name);
                if(ret == OK)
                {
                    Memory.rooms[name].creeps.bouncers++;
                    if(global.config.options.reportHostileNeighbors) { console.log(txt(COLOR.info, 'Spawned bouncer for room ' + name)); }
                }
                else if(ret != ERR_NOT_ENOUGH_ENERGY) { console.log(txt(COLOR.error, 'Error spawning bouncer: ' + ret)); }
                return;
            }
            
            //console.log('SafeMode ',name, ': ', name.controller.safeMode);
...
StructureSpawn.prototype.tryCreateLongDistance = function(roomName)
{
    for(let source of Memory.rooms[roomName].sources)
    {
        if(!Memory.sources[source].longHarvester || Memory.sources[source].longHarvester == 'none')
        {
            ...
            else if(ret != ERR_NOT_ENOUGH_ENERGY) { console.log(txt(COLOR.error, 'Error spawning longHarvester: ' + ret)); }
        }
        if(!Memory.sources[source].longHauler || Memory.sources[source].longHauler == 'none')
        {
            ...
            else if(ret != ERR_NOT_ENOUGH_ENERGY) { console.log(txt(COLOR.error, 'Error spawning longHauler: ' + ret)); }
        }
    }
    return false;
}
...
StructureSpawn.prototype.createRanger = function(energy, targetRoom)
{
    ...
    return this.spawnCreep(mods, name, { memory: {role: 'ranger', target: targetRoom} });
};

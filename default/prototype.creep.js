...
Creep.prototype.withdrawFromTerminal = function()
{
    if (this.room.memory.terminal)
    {
        ...
        {
            if (terminalStructure.store[RESOURCE_ENERGY] <= 500) { return false; }
...
{
    let threat = this.room.find(FIND_HOSTILE_CREEPS, {filter: (c) => c.getActiveBodyparts(ATTACK)}.length, {filter: (c) => c.getActiveBodyparts(RANGED_ATTACK)}).length;
    if (threat > 0)
    {
        console.log(txt(COLOR.warn, this.room.name + ' Threat: ' + threat + ' Running to homeRoom'));
        return true;
    }
    else
    {
